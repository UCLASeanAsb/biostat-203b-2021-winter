...
system.time(labevents_tble <- fread("/usr/203b-data/mimic-iv-derived-data/labevents_filtered_itemid.csv.gz",header = FALSE, col.names = c("subject_id",
              "hadm_id",
              "itemid",
              "charttime",
              "valuenum"),
...
spread(key = itemid, value = valuenum) %>% print(width = Inf) %>% right_join(Uniqicu_stays, by = c("subject_id", "hadm_id")) %>%
  ungroup() %>% 
  rename_at(vars(as.character(labitem_imemid$itemid)), ~labitem_itemid$labitem) %>% print(width = Inf)
