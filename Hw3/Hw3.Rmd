...
icustays_tble %>% group_by(subject_id) %>%
  ...
  show_query() %>%
  collect() %>%
mutate(days_death = as.numeric((ymd_hms(deathtime)-ymd_hms(admittime)),
         units="days")) %>%
  mutate(died_30days = ifelse((days_death<=30 & !is.na(deathtime)), 1, 0)) %>%
saveRDS(str_c("/home/asburysean/biostat-203b-2021-winter/Hw3/mimiciv_shiny/icu_cohort.rds"))
